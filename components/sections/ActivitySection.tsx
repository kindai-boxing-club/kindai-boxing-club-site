...
const events = [
  ...
  { season: "夏季", items: ["関西学生ボクシングオープン戦", "夏合宿"] },
  {
    season: "秋季",
    items: ["関西学生トーナメント戦", "全日本ボクシング選手権大会"],
  },
  { season: "冬季", items: ["送別会", "OB会"] },
];
              ...
              {schedule.map((item, index) => (
                ...
                >
                  <div className="flex flex-col md:flex-row md:items-baseline justify-between gap-2">
                    ...
                    <span className="text-xl font-bold text-gray-800 font-mono mt-2 md:mt-0">
                      {item.time}
                    </span>
                  </div>
                  {item.note && (
                ...
                >
                  ...
                  <ul className="space-y-3">
                    {event.items.map((item, i) => (
                      <li
                        key={i}
                        className="text-sm font-bold text-gray-300 flex items-center gap-3"
                      >
